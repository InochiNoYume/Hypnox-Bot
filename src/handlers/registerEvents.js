...
const TICKET_FORMS = {
  soporte: {
    title: 'FORMULARIO — SOPORTE',
    questions: [
      ['asunto', 'Asunto', '¿En qué necesitas ayuda?', false],
      ['problema', 'Problema', 'Explica brevemente el problema.', true],
      ['detalles', 'Detalles', 'Añade toda la información relevante.', true],
      ['evidencia', 'Evidencia', 'Enlace a imágenes, vídeos u otra evidencia. Es opcional.', false]
    ]
  },
  reporte: {
    title: 'FORMULARIO — REPORTE',
    questions: [
      ['usuario', 'Usuario reportado', 'Indica el usuario o usuarios involucrados.', false],
      ['motivo', 'Motivo', 'Indica el motivo del reporte.', false],
      ['descripcion', 'Descripción', 'Explica detalladamente lo ocurrido.', true],
      ['evidencia', 'Evidencia', 'Enlace a imágenes, vídeos u otra evidencia. Es opcional.', false]
    ]
  },
  alianza: {
    title: 'FORMULARIO — ALIANZA / PARTNER',
    questions: [
      ['tipo', 'Tipo de solicitud', 'Alianza o Partner.', false],
      ['servidor', 'Servidor / Comunidad', 'Nombre y descripción breve de tu comunidad.', true],
      ['propuesta', 'Propuesta', '¿Qué colaboración propones?', true],
      ['miembros', 'Miembros', 'Cantidad aproximada de miembros activos.', false],
      ['contacto', 'Contacto', 'Medio de contacto adicional, si corresponde.', false]
    ]
  },
  contacto: {
    title: 'FORMULARIO — CONTACTO',
    questions: [
      ['asunto', 'Asunto', 'Indica el motivo de tu contacto.', false],
      ['motivo', 'Motivo', 'Explica qué necesitas comunicar al equipo.', true],
      ['detalles', 'Detalles', 'Añade cualquier información adicional.', true],
      ['evidencia', 'Evidencia', 'Enlace a imágenes, vídeos u otra evidencia. Es opcional.', false]
    ]
  }
};
...
function buildTicketModal(type) {
  ...
  for (const [id, label, placeholder, paragraph] of form.questions) {
    const input = new TextInputBuilder()
      ...
      .setRequired(!['evidencia', 'contacto'].includes(id));
...
function buildTicketEmbed(type, user, answers) {
  ...
  embed.addFields(
    ...
    ...form.questions.map(([id, label]) => ({ name: `◆ ${label.toUpperCase()}`, value: answers[id] || 'No proporcionado.', inline: false }))
  );
...
async function createTicketFromModal(interaction, type) {
  ...
  if (!category || category.type !== ChannelType.GuildCategory) {
    return interaction.reply({ content: 'El sistema de tickets no está configurado correctamente: falta la categoría de tickets.', ephemeral: true });
  }
  ...
  const channel = await interaction.guild.channels.create({
    name: ticketChannelName(type, interaction.user.username, interaction.user.id),
  ...
  try {
    ...
    const embed = buildTicketEmbed(type, interaction.user, answers);
    await channel.send({
      content: mention || undefined,
      allowedMentions: mention ? { roles: [mentionRoleId] } : undefined,
      embeds: [embed]
    });
