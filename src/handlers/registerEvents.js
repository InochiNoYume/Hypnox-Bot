  ...
  client.on('interactionCreate', async (interaction) => {
    try {
      if (interaction.isButton() && interaction.customId.startsWith('hypnox_giveaway:')) {
        ...
        if (!giveaway || giveaway.status !== 'active' || new Date(giveaway.ends_at) <= new Date()) {
          return interaction.reply({ content: 'Este sorteo ya finalizó.', ephemeral: true });
        }
      ...
      if (interaction.isStringSelectMenu() && interaction.customId === 'hypnox_ticket_type') {
        ...
        if (guildType !== 'official') {
          return interaction.reply({ content: 'El sistema de tickets solo está disponible en el servidor oficial.', ephemeral: true });
        }

        const type = interaction.values[0];
        const channel = await interaction.guild.channels.create({
          name: `ticket-${interaction.user.username}`.toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 70) || `ticket-${interaction.user.id}`,
      ...
      if (!commandAccessAllowed(interaction, command)) {
