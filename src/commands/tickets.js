...
const { getTicket, getTicketCreatorDiscordId, addTicketEvent, claimTicketAtomic } = require('../services/tickets');
...
const STAFF_ROLE_ENVS = [
  ...
  'OFFICIAL_ROLE_DEVELOPER_ID','OFFICIAL_ROLE_PRODUCER_ID','OFFICIAL_ROLE_SRMOD_ID',
  'OFFICIAL_ROLE_MOD_ID','OFFICIAL_ROLE_TMOD_ID','OFFICIAL_ROLE_HELPER_ID'
];
const TICKET_ACCESS = {
...
const data = new SlashCommandBuilder()
  .setName('tickets').setDescription('Gestiona el sistema de atención del servidor oficial.')
  ...
  .addSubcommand((s) => s.setName('add').setDescription('Añade un usuario al ticket.')
    .addUserOption((o) => o.setName('usuario').setDescription('Usuario que tendrá acceso.').setRequired(true)))
  .addSubcommand((s) => s.setName('remove').setDescription('Retira un usuario del ticket.')
    .addUserOption((o) => o.setName('usuario').setDescription('Usuario al que se retirará el acceso.').setRequired(true)));
...
const select = () => new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
  .setCustomId('hypnox_ticket_type').setPlaceholder('Selecciona el área que necesitas')
  .addOptions(Object.entries(labels).map(([value, label]) => ({ label, value, description: `Abrir una solicitud de ${label.toLowerCase()}` }))));

function isStaff(i) { return STAFF_ROLE_ENVS.some((env) => hasConfiguredRole(i.member, env)); }
function canHandleTicket(i, ticket) { return (TICKET_ACCESS[ticket?.ticket_type] || []).some((env) => hasConfiguredRole(i.member, env)); }

async function hideStaffFromTicket(channel, claimedUserId) {
  for (const env of STAFF_ROLE_ENVS) {
    const roleId = getEnv(env);
    if (!roleId || !channel.guild.roles.cache.has(roleId)) continue;
...
async function execute(i) {
  ...
  try {
    if (sub === 'panel') {
      if (!isStaff(i)) return i.editReply({ content: 'No tienes permisos para publicar el panel.' });
      const id = getEnv('OFFICIAL_CHANNEL_TICKETS_ID');
      const ch = id ? await i.guild.channels.fetch(id).catch(() => null) : i.channel;
      if (!ch?.isTextBased()) return i.editReply({ content: 'Configura OFFICIAL_CHANNEL_TICKETS_ID.' });
      const embed = brandedEmbed('CENTRO DE ATENCIÓN', [
        ...
        '◆ **Bugs / Errores**','> Reporta errores técnicos, fallos de funcionamiento o problemas relacionados con sistemas, contenido o desarrollo.','',
      ...
      return i.editReply({ content: 'Panel de atención publicado.' });
    }

    const ticket = await getTicket(i.channel.id);
    if (!ticket) return i.editReply({ content: 'Este canal no es un ticket activo.' });
    ...
    if (sub === 'claim') {
      const claimed = await claimTicketAtomic(ticket.id, i.user.id);
      if (!claimed) {
        const current = await getTicket(i.channel.id).catch(() => null);
        return i.editReply({ content: current?.assigned_to_discord_user_id ? `Este ticket ya fue reclamado por <@${current.assigned_to_discord_user_id}>.` : 'Este ticket acaba de ser reclamado por otro miembro del equipo.' });
      }
      ...
      return i.editReply({ content: `Ticket reclamado por <@${i.user.id}>.` });
    }
    ...
    if (sub === 'add') {
      ...
      await addTicketEvent(ticket.id, i.user.id, 'message', `Usuario añadido: ${user.id}`).catch(() => {});
      await writeLog({ guild: i.guild, category: 'ticket', action: 'add_member', actorId: i.user.id, targetId: user.id, channelId: i.channel.id });
      return i.editReply({ content: `Se añadió a <@${user.id}>.` });
    }

    await i.channel.permissionOverwrites.delete(user.id).catch(() => {});
    await addTicketEvent(ticket.id, i.user.id, 'message', `Usuario retirado: ${user.id}`).catch(() => {});
    ...
    return i.editReply({ content: `Se retiró a <@${user.id}>.` });
  } catch (error) {
    console.error('[HYPNOX] Ticket command error:', error);
    return i.editReply({ content: 'No se pudo completar la acción. El error fue registrado para revisión.' }).catch(() => {});
  }
}
