...
async function execute(i) {
  ...
  // La apertura del modal debe ser la primera respuesta de la interacción.
  ...
  if (sub === 'claim') {
    ...
    try {
      ...
      if (error?.message === 'TICKET_ALREADY_ASSIGNED' || error?.code === 'P0001') {
        ...
        if (current?.assigned_to_discord_user_id) {
          return i.reply({ content: `Este ticket ya fue reclamado por <@${current.assigned_to_discord_user_id}>.`, flags: EPHEMERAL });
        }
      }
      throw error;
    }
