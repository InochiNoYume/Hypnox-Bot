...
const TICKET_ACCESS = {
  ...
  boost: ['OFFICIAL_ROLE_FOUNDER_ID','OFFICIAL_ROLE_DIRECTOR_ID','OFFICIAL_ROLE_ADMINISTRATOR_ID','OFFICIAL_ROLE_SRMOD_ID','OFFICIAL_ROLE_MOD_ID','OFFICIAL_ROLE_TMOD_ID','OFFICIAL_ROLE_HELPER_ID','OFFICIAL_ROLE_DEVELOPER_ID','OFFICIAL_ROLE_PRODUCER_ID']
};
...
async function buildTranscript(channel, ticket, closedBy = null, reason = null) {
  ...
  for (const message of messages) {
    ...
    if (message.attachments?.size) for (const attachment of message.attachments.values()) lines.push(`  Adjunto: ${attachment.url}`);
  }
...
async function generateTranscript(interaction) {
  ...
  try {
    const ticket = await getTicket(interaction.channelId).catch(() => null);
    if (!ticket) return interaction.editReply({ content: 'Este canal no corresponde a un ticket abierto.' });
    if (!canCloseTicket(interaction, ticket)) return interaction.editReply({ content: 'Solo el personal autorizado para este ticket puede generar el transcript.' });
...
async function deleteOralAssistanceChannel(interaction, ticket) {
  ...
  if (voice) await voice.delete('Ticket cerrado: eliminar Asistencia Oral').catch((error) => console.error(`[HYPNOX] No se pudo eliminar la Asistencia Oral del ticket ${ticket.id}:`, error));
  await updateTicket(ticket.id, { oral_voice_channel_id: null }).catch((error) => console.error(`[HYPNOX] No se pudo limpiar la referencia de Asistencia Oral del ticket ${ticket.id}:`, error));
}
...
async function closeTicketWithReason(interaction, reason) {
  ...
  try {
    ...
    await addTicketEvent(ticket.id, interaction.user.id, 'closed', cleanReason, { assigned_to_discord_user_id: ticket.assigned_to_discord_user_id || null, reason: cleanReason, oral_voice_channel_id: ticket.oral_voice_channel_id || null });
    await writeLog({ guild: interaction.guild, category: 'ticket', action: 'close', actorId: interaction.user.id, channelId: channel.id, message: `Ticket ${ticket.id} cerrado. Motivo: ${cleanReason}`, metadata: { ticketId: ticket.id, reason: cleanReason, assignedTo: ticket.assigned_to_discord_user_id || null, oralVoiceChannelId: ticket.oral_voice_channel_id || null } });
    const creatorId = await getTicketCreatorDiscordId(ticket).catch(() => null);
    ...
    embed.addFields({ name: '◆ SOLICITANTE', value: creatorId ? `<@${creatorId}>` : 'No disponible', inline: true }, { name: '◆ CERRADO POR', value: `<@${interaction.user.id}>`, inline: true }, { name: '◆ MOTIVO', value: cleanReason, inline: false });
    embed.setFooter({ text: 'Hypnox Studios • Sistema de atención' });
