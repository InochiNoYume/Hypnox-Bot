...
async function showCloseModal(interaction) {
  ...
  if (!isConfiguredStaff(interaction)) {
    return interaction.reply({ content: 'No tienes permisos para cerrar tickets.', flags: EPHEMERAL });
  }
  // No hacemos una consulta a Supabase antes de showModal. Esta interacción
  // debe ser reconocida inmediatamente; la validación exacta se hace al enviar el modal.
...
async function closeTicketWithReason(interaction, reason) {
  ...
  try {
    ...
    await interaction.editReply({ content: 'Ticket cerrado. El canal será eliminado en unos segundos.' });
